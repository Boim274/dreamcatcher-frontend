...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Navbar />
      {/* Hero */}
      <section className="relative bg-gradient-to-br from-[#1A1A2E] via-[#1A1A2E] to-purple-900 text-white py-20 lg:py-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 relative text-center">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
            ...
            <span className="text-[#FF6B35]">Dengan AI</span>
          </h1>
          <p className="text-lg text-gray-300 mb-8 max-w-2xl mx-auto">
            Sablon kaos, banner, stiker, dan bordir berkualitas tinggi dengan teknologi AI
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/pesan" className="inline-flex items-center justify-center gap-2 bg-[#FF6B35] hover:bg-[#E55A26] text-white font-semibold py-3 px-8 rounded-lg transition-colors">
              <Icon name="arrow-right" size={20} /> Pesan Sekarang
            </Link>
            <a href="#portfolio" className="inline-flex items-center justify-center gap-2 border-2 border-white text-white hover:bg-white hover:text-[#1A1A2E] font-semibold py-3 px-8 rounded-lg transition-colors">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {services.map((service) => (
                <Card key={service.id} hover className="text-center group">
                  <div className="w-14 h-14 bg-[#FF6B35]/10 rounded-xl flex items-center justify-center mx-auto mb-4 text-2xl group-hover:bg-[#FF6B35] group-hover:text-white transition-all">
                  ...
                  <Link to={`/pesan?service=${service.id}`} className="block w-full border-2 border-[#FF6B35] text-[#FF6B35] font-semibold py-2 rounded-lg hover:bg-[#FF6B35] hover:text-white transition-colors">
                    Pesan
                  </Link>
                </Card>
              ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
            {steps.map((step) => (
              <div key={step.num} className="text-center">
                <div className="w-12 h-12 bg-[#FF6B35] text-white rounded-full flex items-center justify-center text-lg font-bold mx-auto mb-3">
                  {step.num}
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {portfolios.map((portfolio) => (
              <div key={portfolio.id} className="relative group overflow-hidden rounded-xl aspect-square">
                <img src={portfolio.image_url} alt={portfolio.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((f) => (
              <div key={f.title} className="text-center">
                <div className="w-14 h-14 bg-[#FF6B35]/20 rounded-xl flex items-center justify-center mx-auto mb-4">
                  <Icon name={f.icon} size={28} className="text-[#FF6B35]" />
                </div>
      ...
      <section className="py-20 bg-[#FF6B35] text-white text-center">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <Link to="/pesan" className="inline-flex items-center gap-2 bg-white text-[#FF6B35] font-bold py-4 px-8 rounded-lg hover:bg-gray-100 transition-colors">
